...
// 会话角色两种：
//   role:'admin'    —— 经小蓝页 SSO 且小蓝页那边 isAdmin（即站主）
//   role:'creator'  —— 小蓝页登录的创作者（只能改/删 owner===sub 的书）
// 安全要点：
//   1) 管理员仅经小蓝页 SSO 获得（站主身份），不另设站点密码
//   2) 会话存 KV；Cookie: HttpOnly + Secure + SameSite=Lax
...
// 注：管理员登录已改为仅经小蓝页 SSO（站主身份），不再校验站点密码。
...
/** 仅管理员放行（role==='admin'，仅经小蓝页 SSO 获得）；否则 401 */
